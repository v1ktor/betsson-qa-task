...
export class CheckoutPageValidator {
  ...
  public async validateProductsInCheckout(
  ...
  public async validatePaymentAndShippingInfo(
    productsToValidate: ProductKey[],
  ): Promise<void> {
    ...
    await expect(this.selectors.labelPaymentInformationValue).toHaveText(
      "SauceCard #31337",
    );
    await expect(this.selectors.labelShippingInformationValue).toHaveText(
      "Free Pony Express Delivery!",
    );
    await expect(this.selectors.labelPriceTotalValue).toHaveText(
      `Item total: $${totalPrice.toFixed(2)}`,
    );
    await expect(this.selectors.labelPriceTotalTaxValue).toHaveText(
      `Tax: $${totalTax.toFixed(2)}`,
    );
    await expect(this.selectors.labelTotal).toHaveText(
  ...
  public async validateCheckoutIsCompleted(): Promise<void> {
    ...
    await expect(this.selectors.textThankYou).toHaveText(
      "Thank you for your order!",
    );
    await expect(this.selectors.textComplete).toHaveText(
